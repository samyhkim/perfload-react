...
socket.on("connect", () => {
  ...
  // send performance data to socket every second
...
// cpu is all cores; we need the average of all the cores
// which will give us the cpu average
function cpuAverage() {
  const cpus = os.cpus();
  // get ms in each mode, BUT since this number is since
  // reboot so get it now, and get it in 100ms and compare
  let idleMs = 0;
  let totalMs = 0;
...
// bc the times property is time since boot, we will
// get now times, and 100ms from now times; compare
// them to get thee current load
function getCpuLoad() {
